...
export default function WorkflowCard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await AuthenticationService.getWorkflowByName(payload1);
  ...
  }, [taskWf, isOptionSelected]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await AuthenticationService.getWorkflowByName(payload2);
        const temp = JSON.parse(response.data);
        setDefectArray(temp);
        setIsLoading(false);
      } catch (error) {
        ...
        <>
          <div className="flex">
            ...
            <div className="">
              <div className="flex mx-auto gap-4 mt-8 justify-center">
                <div className="py-4">
                  ...
                  <div className="flex gap-6 py-2 mt-2 ml-24">
                    ...
                    <div
                      className={`font-bold ${
                        isSaved || isOptionSelected
                          ? "cursor-not-allowed bg-blue-300"
                          : ""
                      }`}
                      type="submit"
                      disabled={isSaved || isOptionSelected}
                    >
                      <FormDialog
                        prop={
                          <CreateWorkflow
              ...
              <div>
                <h1 className="text-xl pb-5 text-center font-bold ">
                  Workflows
                </h1>
